...
--
--
--  $Id: sgl-vectors.adb,v 2.1 1997/07/12 00:39:33 akonstan Exp $
--
...
package body SGL.Vectors is
   ...
   begin
      ...
      V.VFinish := V.VStart + To_Difference_Type(Copy_Size);
   ...
   function Value(V: in Vector; N: in Size_Type) return Value_Type is
      I : Constant_Iterator := Start(V) + To_Difference_Type(N);
   begin
      return I.Ptr.all;
   end Value;
   ...
   begin
      ...
      return (To_Size_Type(V.VFinish - V.VStart));
   end Size;
   ...
   begin
      ...
      return (To_Size_Type(V.End_Of_Storage - V.VStart));
   end Capacity;
   ...
   function "="(V1, V2: in Vector) return Boolean is
      function Equal is new SGL.Basic_Algorithms.Equal
        (Vector_Input_Constant_Iterators,
         Vector_Input_Constant_Iterators);
   begin
      return (Size(V1) = Size(V2)) and then
   ...
   procedure Assign(V: in out Vector; N: in Size_Type; E: in Value_Type) is
      I : Iterator := Start(V'Unchecked_Access) + To_Difference_Type(N);
   begin
      Construct(I.Ptr, E);
   end Assign;
   ...
   begin
      ...
      if (V.End_Of_Storage - Finish(V) >= N) then
         -- inserting in the middle ?
         if (Finish(V) - Position > N) then
         ...
         Len := Size(V) + Max(Size(V), To_Size_Type(N));
         ...
         DontCare := Uninitialized_Copy(Make_Constant(Position), Finish(V),
                            Tmp + (Position - Start(V) + N));
         Destroy_Sequence(Start(V'Unchecked_Access),
                          Finish(V'Unchecked_Access));
         Value_Allocators.Deallocate(V.VStart.Ptr);
         V.End_Of_Storage := Tmp + To_Difference_Type(Len);
         V.VFinish := Tmp + (To_Difference_Type(Size(V)) + N);
         V.VStart := Tmp;
      end if;
   end Insert_Range;
   ...
   begin
      ...
      if (V.End_Of_Storage - V.VFinish >= N) then
         if (Finish(V) - Position > To_Difference_Type(N)) then
            DontCare := Uninitialized_Copy(Finish(V) - To_Difference_Type(N),
                                           Finish(V),
                                           Finish(V'Unchecked_Access));
            DontCare := Copy_Backward(Make_Constant(Position),
                                      Finish(V) - To_Difference_Type(N),
                                      Finish(V'Unchecked_Access));
            Fill(Position, Position + To_Difference_Type(N), X);
         else
            DontCare := Uninitialized_Copy(Make_Constant(Position), Finish(V),
                                           Position + To_Difference_Type(N));
            Fill(Position, Finish(V'Unchecked_Access), X);
            -- assert Finish(V) > Position
            DontCare := Uninitialized_Fill_N
              (Finish(V'Unchecked_Access),
               N - To_Size_Type(Finish(V) - Position), X);
         end if;
         IncBy(V.VFinish, To_Difference_Type(N));
      else
         Len := Size(V) + Max(Size(V), N);
         Tmp.Ptr := Value_Allocators.Allocate(Len);
         DontCare := Uninitialized_Copy(Start(V), Make_Constant(Position),
                                        Tmp);
         DontCare := Uninitialized_Fill_N
   ...
   begin
      if (V.VFinish /= V.End_Of_Storage) then
         ...
         if (Size(V) = Size_Type'Val(0)) then
         ...
         Value_Allocators.Deallocate(Start(V'Unchecked_Access).Ptr);
         V.End_Of_Storage := Tmp + To_Difference_Type(Len);
         V.VFinish := Tmp + To_Difference_Type(Size(V)) + 1;
         V.VStart := Tmp;
      end if;
   end Insert_Aux;
   ...
   begin
      if (Capacity(V) < N) then
         ...
         Destroy_Sequence(Start(V'Unchecked_Access),
                          Finish(V'Unchecked_Access));
         Value_Allocators.Deallocate(V.VStart.Ptr);
         V.VFinish := Tmp + To_Difference_Type(Size(V));
         V.VStart := Tmp;
         V.End_Of_Storage := Start(V'Unchecked_Access) + To_Difference_Type(N);
      end if;
   end Reserve;
